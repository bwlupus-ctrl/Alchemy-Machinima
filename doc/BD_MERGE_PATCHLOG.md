...
| B11 — hide lookAt/focus broadcast | (this commit) | BD | `BDMergeHideLookAt` | Kyler Eastridge (Black Dragon) | Donor: BD 82477c484d (feature) + 5785bfdb70 (stuck-head fix: one-time IDLE clear-effect send on enable). Ported only the hide-broadcast half — the limit-distance half (`LimitLookAtTarget`/`LimitLookAtTargetDistance`) is already in Alchemy and MORE elaborate than BD's (per-object exceptions). Deliberately not named `EnableLookAtTarget`: Alchemy already uses that key for an unrelated local-indicator-hide feature. Single choke point: all lookAt setters funnel through LLHUDEffectLookAt::setLookAt(). |
