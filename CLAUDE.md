...
- **If the Git Bash tool flakes** with `/usr/bin/bash: -c: line ~85: unexpected EOF while looking for matching '` and **no `[codex]` lines** in the output (Codex never launched — a shell-wrapper/profile failure, not your command; it repeats on retry), launch the identical `node "…" task --write --prompt-file "…" --cwd "…"` via the **PowerShell tool** (`run_in_background`) instead. The companion runs its own subcommands through `powershell.exe` anyway, so this is fully consistent — verified 2026-08-03. Invariant = `--prompt-file` + `run_in_background` + no poll loop; Bash vs PowerShell is only the launcher.
