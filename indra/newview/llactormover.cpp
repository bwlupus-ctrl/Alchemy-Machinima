...
namespace
{
// session-only ordered roster; empty = my avatar
uuid_vec_t sMoverRoster;
...
LLVOAvatar* resolve_actor(const LLUUID& id)
{
    ...
    return (av && !av->isDead()) ? av : nullptr;
}

// the locomotion anim a new Move should start: ANIM_AGENT_WALK unless a valid
// custom override UUID is configured (local playback only)
LLUUID locomotion_anim()
{
...
void LLActorMover::toggleTarget(const LLUUID& id)
{
    ...
    auto it = std::find(sMoverRoster.begin(), sMoverRoster.end(), id);
    if (it != sMoverRoster.end())
    {
        sMoverRoster.erase(it);
    }
    else
    {
        sMoverRoster.push_back(id);
    }
}
...
{
    return id.notNull()
        && std::find(sMoverRoster.begin(), sMoverRoster.end(), id) != sMoverRoster.end();
}
...
void LLActorMover::start(const LLUUID& actor_id)
{
    ...
    mv.mT        = 0.f;
    mv.mAnim     = locomotion_anim();

    // restarting an actor mid-move with a different anim: stop the old one so
    // it doesn't keep looping under the new cycle
    auto old_it = mMoves.find(av->getID());
    if (old_it != mMoves.end() && old_it->second.mAnim != mv.mAnim)
    {
...
{
    if (sMoverRoster.empty())
    {
        start(LLUUID::null);
        return;
    }
    for (const LLUUID& id : sMoverRoster)
    {
        start(id);
    }
}
...
void LLActorMover::renderHeadingPreview()
{
    ...
    uuid_vec_t roster = sMoverRoster;
    if (roster.empty())
    {
