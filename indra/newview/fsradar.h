 ...
 * $/LicenseInfo$
 *
 * Ported from Firestorm (I:\enve, indra/newview/fsradar.h) to Alchemy Machinima
 * as part of the BD/FS -> Alchemy merge campaign, item F3 (Extended Radar +
 * Show Friends Only). See doc/ patch log for details of what was adapted or
 * stripped to fit Alchemy's tree (RLV, ALDerenderList, ALAssetBlocklist, etc).
 */
...
// Const values for avatars at unknown height / range. FS keeps these in
// fscommon.h, which does not exist in Alchemy, so they live here instead.
constexpr F64 AVATAR_UNKNOWN_Z_OFFSET = -1.0;
...
public:
    ...
    // Avatar-level derender helpers (map to ALDerenderList rather than FS's
    // FSAssetBlacklist / LLAvatarActions::derender, which don't exist here).
    static void derenderAvatar(const LLUUID& avatar_id, bool permanent);
    static void derenderAvatars(const uuid_vec_t& avatar_ids, bool permanent);
    ...
    // ---- Show Friends Only -------------------------------------------------
    // Temporarily derenders every nearby non-friend via ALDerenderList (NOT a
    // persistent blocklist entry) and restores them when turned back off, or
    // optionally automatically after the next teleport completes.
    void setShowFriendsOnly(bool enabled);
    bool getShowFriendsOnly() const { return mShowFriendsOnly; }
...
private:
    ...
    void applyShowFriendsOnly();
    void restoreShowFriendsOnly();
    void onShowFriendsOnlySettingChanged();
    ...
    bool                    mShowFriendsOnly;
    uuid_vec_t              mShowFriendsOnlyDerendered; // avatars we auto-derendered; used to restore
    ...
    boost::signals2::connection mShowFriendsOnlyCallbackConnection;
    boost::signals2::connection mShowFriendsOnlyAutoOffCallbackConnection;
    boost::signals2::connection mTeleportFinishedCallbackConnection;
