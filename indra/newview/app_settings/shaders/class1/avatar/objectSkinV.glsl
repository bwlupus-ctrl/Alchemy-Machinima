...
// [BDMerge A5.4-1b] Previous frame's palette (velocity pass only). Donor:
// Black Dragon objectSkinV.glsl. Both the uniform and the function below are
// eliminated at link time in every rigged shader that doesn't call
// getLastObjectSkinnedTransform(), so non-velocity shaders are unaffected.
...
// [BDMerge A5.4-1b] Same blend as getObjectSkinnedTransform, against the
// PREVIOUS frame's palette. Called only by the velocity shaders' HAS_SKIN path.
...
#ifdef IS_AMD_CARD
   mat3x4 dummy1l = lastMatrixPalette[0];
   mat3x4 dummy2l = lastMatrixPalette[MAX_JOINTS_PER_MESH_OBJECT-1];
#endif
