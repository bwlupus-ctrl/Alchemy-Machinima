...
void main()
{
    ...
    if (actor_fx_active)
    {
        ...
        if (actorFxUvTransformEnabled())
        {
            ...
            diff.rgb = texture(diffuseMap, fx_uv).rgb;
        }
        if (actorFxRgbSplitEnabled())
        {
            diff.r = texture(diffuseMap, actorFxRgbSplitUv(fx_uv, -1.0)).r;
            diff.b = texture(diffuseMap, actorFxRgbSplitUv(fx_uv,  1.0)).b;
        }
        actor_fx_styled_linear = actorFxApply(srgb_to_linear(diff.rgb), nvn,
