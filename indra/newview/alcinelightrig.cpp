...
LLUUID rigCookie()
{
    ...
    static LLUUID cached_cookie(DEFAULT_COOKIE);
    const std::string& setting = cookie_setting();
    if (setting != cached_setting)
    {
        LLUUID candidate;
        if (!candidate.set(setting, false) || candidate.isNull())
        {
            candidate.set(DEFAULT_COOKIE, false);
        }
        cached_setting = setting;
        cached_cookie = candidate;
    }
    return cached_cookie;
}

LLUUID rigGoboTexture(S32 index)
{
...
//static
ALCineLightRig& ALCineLightRig::instance()
{
    static ALCineLightRig rig;
    return rig;
}
                                   ...
                                   LLPointer<LLVOVolume>& output)
{
    ...
    if (projector)
    {
        volume->setLightTextureID(rigCookie());
...
bool ALCineLightRig::ensureProjectors()
{
    ...
    for (S32 i = 0; i < LIGHT_COUNT; ++i)
    {
        if (!createEmitter(region, true, mProjectors[i]))
        {
            destroyEmitters();
            return false;
        }
...
bool ALCineLightRig::ensureOmnis()
{
    ...
    for (S32 i = 0; i < LIGHT_COUNT; ++i)
    {
        if (!createEmitter(region, false, mOmnis[i]))
        {
            destroyOmnis();
            return false;
        }
...
void ALCineLightRig::updateShadowPolicy()
{
    static LLCachedControl<S32> shadow_mode(
        gSavedSettings, "CineLightRigShadowMode");
    const S32 mode = std::clamp(static_cast<S32>(shadow_mode), 0, 2);
    for (S32 i = 0; i < LIGHT_COUNT; ++i)
    {
                                ...
                                F32 nominal_radius, F32 subject_scale)
{
    ...
    for (S32 i = 0; i < LIGHT_COUNT; ++i)
    {
        ...
        if (projector && !projector->isDead())
        {
            ...
            const LLUUID cookie = rigGoboTexture(state.mGobo);
...
void ALCineLightRig::tick(F64 presentation_time)
{
    ...
    LLVOAvatar* avatar = mGroupEnabled
        ...
        : LLDirectorCast::instance().resolve(mAnchor);
    if (!avatar)
    {
        destroyEmitters();
        std::memset(&mLastFrame, 0, sizeof(mLastFrame));
    ...
    if (!projectors_ready)
    {
        ...
        {
            projectors_ready = ensureProjectors();
            if (!projectors_ready)
            {
    ...
    applyFrame(mLastFrame, mSmoothedCentre, true_centre,
               mCurrentRadius, globals.mSubjectScale);
    updateShadowPolicy();
    updateProjectorFlags();
    mLastPresentationTime = presentation_time;
}
...
ALCineLightRig::setupNamesGrouped() const
{
    ...
    {
        entries.push_back({ master.mName, true });
    }
