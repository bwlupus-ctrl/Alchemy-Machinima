 ...
 * $/LicenseInfo$
 *
 * Ported from Firestorm (I:\enve, indra/newview/fsradarentry.cpp) to Alchemy
 * Machinima as part of the BD/FS -> Alchemy merge campaign, item F3.
 * API remaps: LLGridManager::isInSecondLife() -> isInProductionGrid() (closest
 * Alchemy equivalent); LLAvatarPropertiesProcessor::sendAvatarNotesRequest()
 * -> sendAvatarLegacyNotesRequest() (Alchemy's name for the same request);
 * FSCommon::isLinden() dropped (unused by any consumer in this port).
 */
    ...
    if (!mPropertiesRequested && mID.notNull())
    {
        if (auto region = gAgent.getRegion())
        {
            if (region->capabilitiesReceived())
            {
                ...
                if (LLGridManager::instance().isInProductionGrid() || region->isCapabilityAvailable(CAPNAME))
                {
                ...
                else
                {
                    ...
                    processor->sendAvatarLegacyNotesRequest(mID);
                }
