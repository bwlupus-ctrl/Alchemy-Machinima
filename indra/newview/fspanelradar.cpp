 ...
 * $/LicenseInfo$
 *
 * Ported from Firestorm (I:\enve, indra/newview/fspanelradar.cpp) to Alchemy
 * Machinima as part of the BD/FS -> Alchemy merge campaign, item F3.
 * Notable remaps: "Open Blocked Residents & Objects" now opens Alchemy's
 * ALFloaterBlocked ("blocked" floater) instead of FS's LLPanelBlockedList
 * (which is compiled out (#if 0) in this tree). FSCommon's filter-editor
 * accelerator check is inlined as a plain Ctrl+F test.
 */
...
{
    // FS uses FSCommon::isFilterEditorKeyCombo() here, which is not present
    // in Alchemy; Ctrl+F is the plain equivalent used elsewhere in the tree.
