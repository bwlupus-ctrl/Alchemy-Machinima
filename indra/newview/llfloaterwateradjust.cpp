/**
 * @file llfloaterwateradjust.cpp
 * @brief Free-floating live water editor with local Windlight preset support.
 *
 * [BDMerge B13] Ported from Black Dragon (donor:
 * I:\black-dragon\indra\newview\llfloaterwateradjust.cpp, NiranV Dean).
 * Adaptations vs donor: gDragonLibrary -> gBDMergeEnvLibrary; the donor's
 * "Water Height" fudge row (RenderWaterHeightFudge +
 * LLViewerRegion::setWaterHeightLocal) is a separate BD feature absent from
 * Alchemy and was dropped; BOOL -> bool at UI enable sites.
 *
...
LLFloaterWaterAdjust::LLFloaterWaterAdjust(const LLSD &key) :
    ...
    mFogDensity(nullptr),
    mUnderwaterMod(nullptr),
    mWaterNormX(nullptr),
    mWaterNormY(nullptr),
    mWaterNormZ(nullptr),
    mFresnelScale(nullptr),
    mFresnelOffset(nullptr),
    mScaleAbove(nullptr),
    mScaleBelow(nullptr),
    mBlurMult(nullptr),
...
void LLFloaterWaterAdjust::onOpen(const LLSD& key)
{
    ...
    gBDMergeEnvLibrary.loadPresetsFromDir(mNameCombo, "water");
    gBDMergeEnvLibrary.addInventoryPresets(mNameCombo, mLiveWater);
}
...
void LLFloaterWaterAdjust::onButtonSave()
{
    ...
    if (mWaterImageChanged && !gBDMergeEnvLibrary.checkPermissions(mTxtNormalMap->getImageItemID()))
        water->setNormalMapID(LLUUID::null);

    gBDMergeEnvLibrary.savePreset(mNameCombo->getValue(), water);
    gBDMergeEnvLibrary.loadPresetsFromDir(mNameCombo, "water");
    gBDMergeEnvLibrary.addInventoryPresets(mNameCombo, water);
}

void LLFloaterWaterAdjust::onButtonDelete()
{
    if (!mLiveWater) return;
    gBDMergeEnvLibrary.deletePreset(mNameCombo->getValue(), "water");
    gBDMergeEnvLibrary.loadPresetsFromDir(mNameCombo, "water");
    gBDMergeEnvLibrary.addInventoryPresets(mNameCombo, mLiveWater);
}
...
void LLFloaterWaterAdjust::loadWaterSettingFromFile(const std::vector<std::string>& filenames)
{
    ...
    gBDMergeEnvLibrary.loadPreset(filename, mLiveWater);
    refresh();
}
...
{
    if (!mLiveWater) return;
    gBDMergeEnvLibrary.onSelectPreset(mNameCombo, mLiveWater);
    refresh();
}
