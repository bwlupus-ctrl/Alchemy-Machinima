            ...
            for (auto& attached_object : attachment->mAttachedObjects)
            {
                ...
                if (visible || !(bridge && bridge->getRadius() < 2.0))
                {
                    ...
                    if (bridge)
                    {
                        ...
                        // stamp the attachment's draw order onto its bridge;
                        // LLPipeline::postSort fans it out to the bridge's alpha
                        // groups, sorting the wearer's whole ensemble at one
                        // avatar depth. HUD alpha sorts in HUD space, so
                        // unrigged HUDs stay unstamped.
